...
class ProductController extends controller {
    ...
    async index(req , res) {
        ...
        if(req.query.category) {
            const products = await Product.paginate({ category: req.query.category }, { page, sort: { createdAt: '-1' }, limit: 15 });
            return res.render('product/products', { products, categories });
        }

        const products = await Product.paginate({}, { page, sort: { createdAt: '-1' }, limit: 15 })
    ...
    async showInfoProductPage(req , res) {
        ...
        const product = await Product.findById(req.query.pi).populate([
            {
                path : 'user',
                select : 'name image'
            }
        ...
        const lastSixProducts = await Product.find().sort({ createdAt: '-1' }).limit(6);
        let sameCategoryProducts = await Product.find({ category: product.category }).limit(6).select('name desc image price');
        ...
        if(req.isAuthenticated()) {
            ...
            user.cart.forEach(p => {
                if(p.id == product.id) cartExist = true;
            })
            ...
            res.render('product/single-product', { product, cartExist, favouriteExist, lastSixProducts, sameCategoryProducts });

        } else {
            res.render('product/single-product', { product, lastSixProducts, sameCategoryProducts });
        }
    ...
    async showUserCartPage(req , res) {
        ...
        res.render('product/user-cart', { cart: user.cart, layout: 'dashboard/master', products });
    }
    ...
    async favouriteProducts(req , res) {
        ...
        let products = await Product.paginate({ _id: productIDs }, { page, limit: 5, populate: { path: 'category', select: 'name' }});

        res.render('product/favouriteProducts', { products, layout: 'dashboard/master' })
    }

    async boughtProducts(req , res) {
        if(! this.isMongoId(req.params.id)) return this.back(req, res);

        const user = await User.findById(req.user.id).populate({ path: 'products.id', });
    ...
    async countUpdater(req, res, inc) {
        ...
        let newCount = 0;
        // array is the cart of the user(it is used because I cant update the user cart directly!)
        let array = [];
        if(! this.isMongoId(req.params.id)) return this.back(req, res);
        ...
        array.forEach(async (p, i) => {
            ...
            if(p.id == req.params.id) {
                if(p.count > 0 && p.count !== 1) {
                    ...
                    await User.updateOne({ _id: req.user.id }, { $set: { cart: array }});
                    res.send(`${newCount}`);
    ...
    async searchInProduct(req, res) {
        ...
        let products = await Product.paginate({ _id: array }, { page, sort: { createdAt: 1 }, limit: 15  })
        let categories = await Category.find({ parent : null }).populate('childs').exec();

        res.render('product/products', { products, categories, searchedText: text });
    }
    ...
    async payment(req, res , next) {
        try {
            ...
            if(notavailable) {
                return this.alertAndBack(req, res, {
                    title : 'دقت کنید',
                    message : 'شما محصولی را در سبد خرید خود دارید که فرد دیگری این محصول را زودتز از شما خریداری کرده و هم اکنون موجودی این محصول در انبار به پایان رسیده است. اگر هنوز هم قصد خرید این محصول را دارید، ابتدا این محصول را از سبد خرید خود حذف کنید و منتظر موجود شدن این محصول بمانید.از خرید و اعتماد شما متشکریم.',
                    type : 'error',
                    timer: 30000,
                    button : 'خیلی خوب'
                });
    ...
    async comment(req, res , next) {
        try {
            ...
            return this.alertAndBack(req, res, {
                title: 'نظر شما با موفقیت ثبت شد و پس از تایید شدن به نمایش در می آید. از توجه شما متشکریم.',
                type: 'success',
                toast: true,
                position: 'center'
            });
